...
{
	unsigned long magic;	// must always be at the start of any input stack entry
	...
	*p = t;
	...
	while (np)
	{
		...
		for (i = 0; set_pragmas[i].setstr; i++)
		{
			...
			{
				pse = (struct pragma_stack_entry *)input_stack_pop(as, 1111638594UL, pragma_stack_compare, (void *)&(set_pragmas[i].flag));
				if (pse)
				{
